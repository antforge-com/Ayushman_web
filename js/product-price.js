...
async function handleCalculation(shouldSave) {
    ...
    try {
        ...
        // Calculate number of bottles and their cost from bottle rows
        const bottleRows = materialRows.filter(r => r.type === 'bottle');
        const numBottles = bottleRows.reduce((sum, r) => sum + (r.quantity || 0), 0);
        const bottleCost = bottleRows.reduce((sum, r) => sum + (r.totalCost || 0), 0);
        ...
        if (numBottles <= 0) {
            pricingResultsDiv.innerHTML = `
                <div style="padding:1rem;background:#fee2e2;color:#991b1b;border-radius:8px;text-align:center;">
                    Please add at least one bottle with a quantity greater than 0.
                </div>
            `;
            pricingResultsDiv.style.display = 'block';
            return;
        ...
        // Calculate ingredient cost separately
        const ingredientCost = materialRows
                ...
                bottleInfo: {
                    bottles: bottleRows.map(row => {
                        ...
                        return {
                            ...
                            bottleName: bottle ? bottle.name : 'Unknown',
                            quantity: row.quantity,
                            costPerBottle: row.costPerUnit,
                            totalCost: row.totalCost,
                        };
                    }),
